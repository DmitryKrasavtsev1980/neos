...
// src/PlanPlugin.ts
import { AbstractConfigurablePlugin, events as events2, utils as utils2 } from "@photo-sphere-viewer/core";
...
function getStyle(defaultStyle, style, isHover) {
  return {
    ...
    borderColor: isHover ? style.hoverBorderColor ?? style.borderColor ?? defaultStyle.hoverBorderColor ?? defaultStyle.borderColor : style.borderColor ?? defaultStyle.borderColor ?? "transparent",
    borderSize: isHover ? style.hoverBorderSize ?? defaultStyle.hoverBorderSize : style.borderSize ?? defaultStyle.borderSize
  };
}
...
var PlanComponent = class extends AbstractComponent2 {
  ...
  handleEvent(e) {
    if (utils.getMatchingTarget(e, `.${CONSTANTS2.CAPTURE_EVENTS_CLASS}:not(.psv-plan)`)) {
      return;
    }
    switch (e.type) {
  ...
  __onKeyPress(e) {
    if (e.matches(CONSTANTS2.KEY_CODES.Escape)) {
      this.toggleMaximized();
      return;
    }
  }
...
var _PlanPlugin = class _PlanPlugin extends AbstractConfigurablePlugin {
  static withConfig(config) {
    return [_PlanPlugin, config];
  }
...
_PlanPlugin.id = "plan";
_PlanPlugin.VERSION = "5.14.0";
_PlanPlugin.configParser = getConfig;
_PlanPlugin.readonlyOptions = [
  "visibleOnLoad",
  "defaultZoom",
  "layers",
  "configureLeaflet",
